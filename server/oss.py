...
@app.route('/api/forge/oss/buckets', methods=['GET'])
def buckets ():
	if not request.args ['id']:
		abort (400)
		abort (Response ('Invalid parameter'))
...
#                               })
#                           })
#                           res.json(list);});         }).catch(function(error) {
...
#             res.status(200).end();}).catch(function(error) {
...
#             objects.uploadObject(bucketKey, req.file.originalname, filecontent.length, filecontent, {
#             }, oauth.OAuthClient(), credentials)
#             .then(function(object) {
#                 res.end(); }).catch(function(error) {
